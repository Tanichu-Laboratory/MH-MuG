...
def run():
    #---------------- set params -----------------#
    data_type = ["train", "val", "test"] #choose from ["train", "val", "test"]
    data_name = "classical_jazz_1024"
    npy_dir = f"data/npy/{data_name}"
    audio_dir = f"data/audio/{data_name}"

    #---------------- convert -----------------#
    for dt in data_type:
        npy_dir_tmp = f"{npy_dir}/{dt}"
        audio_dir_tmp = f"{audio_dir}/{dt}"
        os.makedirs(audio_dir_tmp, exist_ok=True)
        convert_all_npydata(npy_dir_tmp, audio_dir_tmp)
