...
#add
class DiTRotary_textcond(nn.Module):
    """
    Diffusion model with a Transformer backbone, with rotary position embedding.
    Use 1D position encoding, patchify is set to False
    ...
    def __init__(
        ...
        self.t_embedder = TimestepEmbedder(hidden_size)
        self.num_classes = num_classes
        self.y_embedder = nn.Linear(512, hidden_size)
...
def DiTRotary_XL_8_textcond(**kwargs):
    return DiTRotary_textcond(depth=28, hidden_size=1152, patch_size=8, num_heads=16, **kwargs)
